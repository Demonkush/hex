...
SWEP.Primary.Sound          = Sound("wc3sound/ChimaeraAlternateMissileHit1.wav")
...
-- Primary Low: Venombolt
-- Primary Norm: Poison Bolt
-- Primary High: Plague Missile
-- Secondary Norm: Toxic Bomb
-- Secondary High: Super Toxic Bomb
...
function SWEP:MainAbility()
    ...
    if self.Owner:GetNWInt("Overpowered") == 1 then
        self:EmitSound("wc3sound/ChimaeraAlternateMissileHit1.wav",85,math.random(125,135))
...
function SWEP:DoPrimaryAttack(output)
    local projent = "ent_hex_proj_poisonbolt"
    local gravity = true
...
function SWEP:DoSecondaryAttack(output)
    local projent = "ent_hex_proj_poisonbomb"
    local gravity = true
...
-- Effect taken from the Nomad laser smg weapon. Workshop id: 104516493
    ...
    effects.Register( EFFECT, "MuzzleWand004" )
end