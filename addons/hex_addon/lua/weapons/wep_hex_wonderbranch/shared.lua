...
-- Wonderbranch -- Elements: Magic --
-- Primary Low: Magic Spark
-- Primary Norm: Arcane Bolt
...
function SWEP:MainAbility()
    ...
    if self.Owner:GetNWInt("Magesoul") == 1 then
        ...
        self:EmitSound("wc3sound/exp/PriestCastAttack1.wav",85,math.random(75,85))
...
function SWEP:MagesoulAbility()
    local projent = "ent_hex_projectile_arcanebomb"
    local atkspeeddelay = 0.2
...
function SWEP:DoPrimaryAttack(output)
    local projent = "ent_hex_projectile_arcanebolt"
    local gravity = true
    ...
    for i=1,num do
        if SERVER then
            ...
            phys:SetVelocity(phys:GetVelocity()+self.Owner:GetAimVector()+(VectorRand()*projrec*115))
