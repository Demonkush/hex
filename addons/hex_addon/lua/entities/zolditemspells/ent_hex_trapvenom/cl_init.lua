...
function ENT:Draw()
    ...
    local ind = "hexgm/ui/aura_poison.png"

    if tr.Hit then
        cam.Start3D2D( tr.HitPos+Vector(0,0,1), Angle(0,0,0), 1 )
            surface.SetDrawColor( Color(215,255,155,5) )
	...
	local particle = emitter:Add("hexgm/sprites/hexbit12", self:GetPos() + Vector(math.random(-5,5),math.random(-5,5),math.random(0,25)) )
		particle:SetVelocity(Vector(0,0,64))
		particle:SetDieTime(math.random(0.5,1.5))
		particle:SetStartSize(math.random(3,5))
		particle:SetStartAlpha(0)
		particle:SetEndAlpha(math.random(25,55))
		particle:SetColor( 155, math.random(215,255), 55 ) 
