...
function ENT:Initialize()
	...
	if SERVER then
		util.SpriteTrail(self.Entity, 0, Color(115,155,255,55), false, 10, 2, 0.25, 15, "trails/laser.vmt")
	end
...
function ENT:ExplodeLow()
	self:DoFX("low")
	self.Entity:EmitSound("wc3sound/exp/FrostBoltHit1.wav",75,math.random(125,130))
...
function ENT:ExplodeNormal()
	self:DoFX("normal")
	self.Entity:EmitSound("wc3sound/exp/FrostBoltHit1.wav",75,math.random(115,120))
	for _, v in ipairs(ents.FindInSphere(self:GetPos(),125)) do
		if v != self:GetOwner() then
			local dmginfo = DamageInfo()
			dmginfo:SetAttacker(self:GetOwner())
			dmginfo:SetInflictor(self)
			dmginfo:SetDamage(math.random(11,15))
			dmginfo:SetDamageElement("frost")
			v:TakeDamageInfo(dmginfo)
		end
	end
...
function ENT:ExplodeHigh()
	self:DoFX("high")
	self.Entity:EmitSound("wc3sound/exp/FrostBoltHit1.wav",75,math.random(110,115))
	for _, v in ipairs(ents.FindInSphere(self:GetPos(),145)) do
		if v != self:GetOwner() then
			local dmginfo = DamageInfo()
			dmginfo:SetAttacker(self:GetOwner())
			dmginfo:SetInflictor(self)
			dmginfo:SetDamage(math.random(13,17))
			dmginfo:SetDamageElement("frost")
			v:TakeDamageInfo(dmginfo)
		end
	end
...
function ENT:DoFX(power)
	...
	elseif power == "normal" then
		...
		fx2:SetAngles(Angle(115,155,255))
